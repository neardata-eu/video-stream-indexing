...
latency_log = None
global_var = {"counter": 0, "milvus_counter": 0}
...
def add_probe(pipeline, element_name, callback, pad_name="sink", probe_type=Gst.PadProbeType.BUFFER, model=None, milvus=None, global_milvus=None, device=None, key_frames=None):
    ...
    sinkpad.add_probe(probe_type, callback, {"model": model, "milvus": milvus, "global_milvus": global_milvus, "device": device, "key_frames": key_frames})
...
def set_event_message_meta_probe(pad, info, u_data):
    ...
    if gst_buffer:
        ...
        if not gst_buffer.has_flags(Gst.BufferFlags.DELTA_UNIT) and meta.duration != Gst.CLOCK_TIME_NONE:   
            ...
            if global_var["counter"] % u_data["key_frames"] == 0:
                ...
                insert_data = [[global_var["milvus_counter"]], embeds, [str(meta.timestamp)]]
                ...
                global_var["milvus_counter"] += 1
                
                # Log the latency
                if (DO_LATENCY_LOG):
                    latency_log.write(str(global_var["counter"]) + "," +
                                    str(global_var["milvus_counter"]) + "," +
...
def main():
    ...
    parser.add_argument('--key_frames', default=25)
    ...
    if (DO_LATENCY_LOG):
        ...
        latency_log.write("frame number,milvus pk,e2e latency(ms),model inference(ms),milvus transfer(ms),pts timestamp, initial timestamp, embedding timestamp, milvus timestamp\n")
    ...
    if sink:
        add_probe(pipeline, "sink", set_event_message_meta_probe, pad_name='sink', model=model, milvus=milvus_collection, global_milvus=milvus_global_collection, device=device, key_frames=int(args.key_frames))
    ...
    total_data_processed = global_var["milvus_counter"]*global_var["size"][0]*global_var["size"][1]*global_var["size"][2]/1024/1024
    config = {
        "stream": args.stream,
        "log_path": log_path,
        ...
        "total_frames_sent_to_milvus": global_var["milvus_counter"],
        ...
        "throughput_processed_mbps": total_data_processed/pipeline_duration,
